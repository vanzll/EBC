...
class Diffusion(nn.Module):
    ...
    def calc_reward(self, x_start):
        ...
        # 创建所有时间步
        t = torch.arange(self.n_timesteps-1, -1, -1, device=device)  # [T]
        t_batch = t.repeat(batch_size, 1)  # [B, T]

        # 创建所有噪声
        noise = torch.randn(batch_size, self.n_timesteps, *x_start.shape[1:], device=device)  # [B, T, ...]

        # 展平batch和time维度以进行并行处理
        t_flat = t_batch.reshape(-1)  # [B*T]
        x_start_expanded = x_start.unsqueeze(1).expand(-1, self.n_timesteps, -1)  # [B, T, ...]
        x_start_flat = x_start_expanded.reshape(-1, *x_start.shape[1:])  # [B*T, ...]
        noise_flat = noise.reshape(-1, *x_start.shape[1:])  # [B*T, ...]

        # 并行计算所有时间步的x_t
        x_t_flat = self.q_sample(x_start=x_start_flat, t=t_flat, noise=noise_flat)  # [B*T, ...]

        with torch.no_grad():
            # 并行预测所有噪声
            x_recon_flat = self.model(x_t_flat, t_flat)  # [B*T, ...]
            
            # 计算损失
            l2_flat = torch.pow((x_recon_flat - noise_flat), 2)  # [B*T, ...]
            loss_t_flat = torch.exp(-l2_flat)  # [B*T, ...]

            # 重塑回 [B, T, ...]
            loss_t = loss_t_flat.reshape(batch_size, self.n_timesteps, -1)  # [B, T, ...]

        # 计算最终的奖励
        disc_cost = loss_t.sum(dim=1) / self.n_timesteps  # [B, ...]
        disc_cost = disc_cost.sum(dim=1) / x_start.shape[1]  # [B]
